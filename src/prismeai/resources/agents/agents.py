...
class Agents:
    ...
    def _path(self, *segments: str) -> str:
        parts = "/".join(segments)
        return f"/workspaces/slug:{self._workspace_id}/webhooks/v1/{parts}" if parts else f"/workspaces/slug:{self._workspace_id}/webhooks/v1"
    ...
    def import_(self, agents_md: Any) -> Any:
        """Import an agent from AGENTS.md format (YAML front matter + markdown instructions)."""
        if isinstance(agents_md, str):
            return self._client.post(self._path("agents", "import"), json={"agents_md": agents_md})
        # Legacy: if dict passed, wrap it
        return self._client.post(self._path("agents", "import"), json={"agents_md": agents_md})

    def retention(self, agent_id: str, **kwargs: Any) -> Any:
        return self._client.patch(self._path("agents", agent_id, "retention"), json=kwargs)
...
class AsyncAgents:
    ...
    async def import_(self, agents_md: Any) -> Any:
        """Import an agent from AGENTS.md format (YAML front matter + markdown instructions)."""
        if isinstance(agents_md, str):
            return await self._client.post(self._path("agents", "import"), json={"agents_md": agents_md})
        return await self._client.post(self._path("agents", "import"), json={"agents_md": agents_md})

    async def retention(self, agent_id: str, **kwargs: Any) -> Any:
        return await self._client.patch(self._path("agents", agent_id, "retention"), json=kwargs)
